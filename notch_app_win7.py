# -*- coding: utf-8 -*-
"""
ガラス基板ノッチ判定ツール Win7/Python3.8 版（Basler撮像 + PLC返答 + 自動再接続 + 一時ファイル上限 + CSVローテーション + 連続Grab）
- 解析は直線近似のみ（RU/RD）
...
import csv, os, re, traceback, tempfile, time, json, glob, threading, sys, logging, faulthandler, signal, atexit, queue, ctypes
...
def _fault_log_path_for_date(day_str: str):
    return os.path.join(LOG_DIR, f"notch_app_fault_{day_str}.log")

def _cleanup_old_log_files(retention_days: int = LOG_RETENTION_DAYS):
    try:
        cutoff = time.time() - (max(1, int(retention_days)) * 86400)
        patterns = [
            os.path.join(LOG_DIR, "notch_app.log*"),
            os.path.join(LOG_DIR, "notch_app_fault*.log*"),
        ]
        for pat in patterns:
...
def _setup_fault_handler():
    global _FAULT_HANDLER_FILE, _FAULT_HANDLER_DATE
    try:
        day_str = time.strftime("%Y-%m-%d")
        if _FAULT_HANDLER_FILE is not None and _FAULT_HANDLER_DATE == day_str:
            return _FAULT_HANDLER_FILE
        ...
        f = open(_fault_log_path_for_date(day_str), "a", encoding="utf-8")
        faulthandler.enable(file=f, all_threads=True)
        # UIフリーズ疑い時にスタックを定期ダンプできるようにする（調査用途）
        faulthandler.dump_traceback_later(60, repeat=True, file=f)
        _FAULT_HANDLER_FILE = f
        _FAULT_HANDLER_DATE = day_str
        _cleanup_old_log_files()
        LOGGER.info("Fault handler enabled: %s", _fault_log_path_for_date(day_str))
        return f
    except Exception:
...
def _enable_win_dpi_awareness():
    """Windows表示スケーリング対策（Win7互換APIを優先）。"""
    try:
        if os.name != "nt":
            return
        ...
        if hasattr(user32, "SetProcessDPIAware"):
            ...
            LOGGER.info("DPI awareness enabled via SetProcessDPIAware")
    except Exception:
...
RESULT_CSV_HEADER=["filename","folderpath","trim_ratio_x","trim_ratio_y","diff_thresh","flip_horizontal",
...
class NotchApp(ttk.Window):
    def __init__(self):
        ...
        self._auto_state = tk.StringVar(value="Auto: 起動")
    ...
    def _save_cfg(self, context="設定保存", notify_ok=False):
        ok = self.cfg.save()
        if ok:
            if notify_ok:
                self._post_status(f"{context}: 保存しました")
        else:
            self._post_status(f"[エラー] {context}: 保存に失敗しました（log参照）")
        return ok


    def _set_auto_state(self, text):
        def _update():
            try:
                self._auto_state.set(f"Auto: {text}")
            except Exception:
                pass
        try:
    ...
    def _ensure_directory_writable(self, path, label):
        ...
        try:
            ...
            test_file = os.path.join(path, f".write_test_{time.time_ns()}")
            with open(test_file, "w", encoding="utf-8") as f:
                f.write("ok")
            os.remove(test_file)
            return True
        except Exception as e:
            self._post_status(f"[エラー] {label} へ書込みできません: {e}")
            return False
    ...
    def _build_ui(self):
        ...
        hdr=ttk.Frame(logframe); hdr.pack(fill="x")
        ttk.Label(hdr, text="ステータス（最新5件）", bootstyle=INFO).pack(side="left", anchor="w")
        ttk.Label(hdr, textvariable=self._auto_state, bootstyle=SECONDARY).pack(side="right")
    ...
    def _collect_analysis_params(self):
        return {
    ...
    def _build_result_csv_row(self, path, res, result_img_path):
        params = self._collect_analysis_params()
        return [
            os.path.basename(path),
            os.path.dirname(path),
            params["trim_ratio_x"], params["trim_ratio_y"], params["diff_thresh"], int(params["flip_horizontal"]),
            params["band_top"], params["band_right"], params["band_bottom"],
            params["corner_exclude_x_px"], params["corner_exclude_y_px"],
    ...
    def _build_error_csv_row(self, path, status):
        params = self._collect_analysis_params()
        folder = os.path.dirname(path) if path else (self.cfg.plc_shot_dir or tempfile.gettempdir())
    ...
    def on_pick_outdir(self):
        ...
        if d:
            self.output_dir=d
            self._sync_cfg_from_vars(); self._save_cfg("出力先設定")
    ...
    def on_preview(self):
        try:
            ...
            res=analyze_image(path, **self._collect_analysis_params())
        ...
        try:
            ...
            for path in paths_snapshot:
                try:
                    ...
                    row=self._build_result_csv_row(path, res, out_path)
    ...
    def _show_preview(self, path, result_dict):
        ...
        try:
            self.after(0, self._refresh_result_text)
        except Exception:
            pass
    ...
    def on_plc_connect(self):
        if (self.plc_thread and self.plc_thread.is_alive()) or (self.plc_worker_thread and self.plc_worker_thread.is_alive()):
            self._post_status("トリガ監視中はPLC再接続できません。先に監視停止してください。")
            return
        if self.plc_connected:
            self._post_status("PLCは既に接続済みです（再接続は実行しません）")
            return
        try:
    ...
    def on_plc_watch_start(self):
        ...
        if not self._ensure_directory_writable(self.output_dir, "出力先"):
            return
        if not self._ensure_directory_writable(self.cfg.plc_shot_dir, "PLC保存先"):
            return
    ...
    def _handle_plc_trigger(self, trig_item):
        ...
        if ok_capture and img_tmp_path:
            try:
                t_an = time.monotonic()
                res=analyze_image(img_tmp_path, **self._collect_analysis_params())
                ...
                row=self._build_result_csv_row(img_tmp_path, res, result_img_path)
                self._append_result_csv(csv_path, header, row)
                ...
                t_csv = time.monotonic()
                row=self._build_error_csv_row(img_tmp_path, "ERROR_ANALYZE")
                self._append_result_csv(csv_path, header, row)
                csv_ms = (time.monotonic() - t_csv) * 1000.0
        else:
            to_error=True
            t_csv = time.monotonic()
            status = "ERROR_TIMEOUT" if (error_type=="TIMEOUT") else "ERROR_CAPTURE"
            row=self._build_error_csv_row("", status)
            self._append_result_csv(csv_path, header, row)
            csv_ms = (time.monotonic() - t_csv) * 1000.0
    ...
    def _auto_manager_loop(self):
        ...
        while True:
            try:
                _setup_fault_handler()
                if bool(self.var_auto_reconnect.get()) and not self.manual_disconnected:
                    self._set_auto_state("有効")
                    # PLC 自動接続
                    if not self.plc_connected:
                        try:
                            self._set_auto_state("PLC接続試行")
                            self.on_plc_connect()
                            plc_backoff=1.0
                            self._set_auto_state("PLC接続済み")
                        except Exception as e:
                    ...
                    if self.plc_connected:
                        if self.video_running:
                            ...
                            if not HAS_PYLYON:
                                ...
                                if need_cam:
                                    try:
                                        self._set_auto_state("カメラ接続試行")
                                        self._cam_open_if_needed()
                                        cam_backoff=1.0
                                        self._post_status("カメラ接続OK")
                                        self._set_auto_state("監視待機")
                                    except Exception as e:
                                        self._cam_close_and_null()
                    ...
                    if self.plc_connected and (self.basler is not None) and bool(self.var_auto_watch_on_ready.get()) and not self.manual_stopped:
                        if not (self.plc_thread and self.plc_thread.is_alive()):
                            self._set_auto_state("監視自動開始")
                            self.on_plc_watch_start()

                # 監視中の断検知（PLC）
                if not bool(self.var_auto_reconnect.get()):
                    self._set_auto_state("無効")
                elif self.manual_disconnected:
                    self._set_auto_state("手動切断中")
                ...
                if self.plc_thread and self.plc_thread.is_alive():
                    try:
                        ...
                        self.plc_connected=False
                        self._set_auto_state("通信断検知")
                time.sleep(0.8)
            except Exception as e:
                self._set_auto_state("例外")
    ...
    def _append_result_csv(self, csv_path, header, row):
        with self._csv_lock:
            ...
            meta = self._csv_meta_cache.get(csv_path)
            exists = os.path.isfile(csv_path)
            ...
            if meta is None:
                ...
                if exists:
                    try:
                        ...
                        header_ok = False
                meta = {"count": count, "header_ok": header_ok}
                self._csv_meta_cache[csv_path] = meta

            d = os.path.dirname(csv_path)
            if d:
                os.makedirs(d, exist_ok=True)

            if meta["header_ok"] and meta["count"] < max_records:
            ...
            self._csv_meta_cache[csv_path] = {"count": len(rows), "header_ok": True}
    ...
    def destroy(self):
        try:
            ...
            self._sync_cfg_from_vars(); self._save_cfg("終了時設定保存")
            try:
                faulthandler.cancel_dump_traceback_later()
            except Exception:
                pass
