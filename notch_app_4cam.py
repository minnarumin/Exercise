...
class ZoomPanCanvas(tk.Canvas):
    def __init__(self, master, bg="#222", **kw):
        ...
        self.min_zoom = 0.05
        self.max_zoom = 10.0
        self.offset = np.array([0.0, 0.0], np.float32)
    ...
    def fit_to_window(self):
        ...
        self.zoom = max(self.min_zoom, min(self.max_zoom, scale))
        self.offset = np.array([(cw - iw * self.zoom) / 2.0, (ch - ih * self.zoom) / 2.0], np.float32)
        self._render()
    ...
    def _on_press(self, e):
        self._drag_start = np.array([e.x, e.y], np.float32)

    def _on_drag(self, e):
        if self._drag_start is None:
            return
        cur = np.array([e.x, e.y], np.float32)
    ...
    def _zoom_at(self, factor, cx, cy):
        if self._img_pil is None:
            return
        old = self.zoom
        new = max(self.min_zoom, min(self.max_zoom, self.zoom * factor))
        if abs(new - old) < 1e-6:
            return
        img_pt = (np.array([cx, cy], np.float32) - self.offset) / old
        self.zoom = new
        self.offset = np.array([cx, cy], np.float32) - img_pt * self.zoom
        self._render()
    ...
    def _render(self):
        ...
        if self._img_pil is None:
            return
        iw, ih = self._img_pil.size
        disp = self._img_pil.resize((max(1, int(iw * self.zoom)), max(1, int(ih * self.zoom))), Image.Resampling.LANCZOS)
        self._img_tk = ImageTk.PhotoImage(disp)
        self._img_id = self.create_image(float(self.offset[0]), float(self.offset[1]), image=self._img_tk, anchor="nw")
...
def _default_cams():
    return [
        CameraParam(camera_index=0, dev_trig="M100", dev_ru="M101", dev_rd="M102", dev_done="M111", dev_err_to="M121", dev_err_an="M131", csv_filename="result_cam1.csv"),
        CameraParam(camera_index=1, dev_trig="M200", dev_ru="M103", dev_rd="M104", dev_done="M112", dev_err_to="M122", dev_err_an="M132", csv_filename="result_cam2.csv"),
        CameraParam(camera_index=2, dev_trig="M300", dev_ru="M105", dev_rd="M106", dev_done="M113", dev_err_to="M123", dev_err_an="M133", csv_filename="result_cam3.csv"),
        CameraParam(camera_index=3, dev_trig="M400", dev_ru="M107", dev_rd="M108", dev_done="M114", dev_err_to="M124", dev_err_an="M134", csv_filename="result_cam4.csv"),
    ]
...
class AppConfig4Cam:
    ...
    def load():
        try:
            ...
            data["cams"] = cams[:4]
            allowed = set(AppConfig4Cam.__dataclass_fields__.keys()) - {"cams"}
            cfg_base = {k: v for k, v in data.items() if k in allowed}
            return AppConfig4Cam(**cfg_base, cams=data["cams"])
        except Exception:
...
class NotchApp4Cam(ttk.Window):
    def __init__(self):
        ...
        self.plc_stop = False
        self.prev_trigs = [False, False, False, False]
        ...
        self._status_lines = deque(maxlen=200)
        self._last_file_paths = [None, None, None, None]
        self._reanalysis_jobs = [None, None, None, None]
        self.trigger_queue = queue.Queue(maxsize=200)
        self.trigger_worker_thread = None
        ...
        for c in self.cfg.cams:
            self.cam_vars.append({
                ...
                "dev_trig": tk.StringVar(value=c.dev_trig),
        ...
        self.after(300, self._start_auto_manager)
    ...
    def _build_ui(self):
        ...
        path_info = ttk.Frame(root)
        path_info.pack(fill="x", pady=(6, 0))
        self.lbl_output_path = ttk.Label(path_info, text="", bootstyle=INFO)
        self.lbl_output_path.pack(anchor="w")
        self.lbl_plc_shot_path = ttk.Label(path_info, text="", bootstyle=INFO)
        self.lbl_plc_shot_path.pack(anchor="w")
    ...
    def _build_menu(self):
        ...
        menu_setting.add_command(label="解析プリセット保存...", command=self._save_analysis_preset)
        menu_setting.add_command(label="解析プリセット読込...", command=self._load_analysis_preset)
    ...
    def _open_plc_common_settings(self):
        ...
        ttk.Label(r2, text="HB Dデバイス").pack(side="left", padx=(10, 0))
        ttk.Entry(r2, textvariable=self.var_dev_alive, width=10).pack(side="left", padx=4)
        ttk.Label(r2, text="周期[ms]").pack(side="left")
        ttk.Entry(r2, textvariable=self.var_alive_ms, width=8).pack(side="left", padx=4)
        ttk.Label(r2, text="増分").pack(side="left")
        ttk.Entry(r2, textvariable=self.var_alive_step, width=6).pack(side="left", padx=4)
        ...
        ttk.Entry(r4, textvariable=self.var_output_dir, width=33).pack(side="left", padx=4)
        ttk.Button(r4, text="参照...", command=self._pick_output_dir).pack(side="left", padx=(0, 8))
        ttk.Label(r4, text="PLC保存先").pack(side="left")
        ttk.Entry(r4, textvariable=self.var_plc_shot_dir, width=24).pack(side="left", padx=4)
        ttk.Button(r4, text="参照...", command=self._pick_plc_shot_dir).pack(side="left")
        ttk.Label(frm, text="出力先: 結果CSVの保存先 / PLC保存先: PLCトリガ撮像画像(PLCshot_*.png)の保存先", bootstyle=INFO).pack(fill="x", pady=(2, 0))
    ...
    def _pick_output_dir(self):
        d = filedialog.askdirectory(title="結果CSVの出力先フォルダを選択")
        if d:
            self.var_output_dir.set(d)

    def _pick_plc_shot_dir(self):
        d = filedialog.askdirectory(title="PLCトリガ撮像画像の保存先フォルダを選択")
        if d:
            self.var_plc_shot_dir.set(d)

    def _refresh_path_labels(self):
        out_dir = self.var_output_dir.get().strip() or "(未設定: PLC保存先→Tempを使用)"
        plc_dir = self.var_plc_shot_dir.get().strip() or "(未設定: Tempを使用)"
        if hasattr(self, "lbl_output_path"):
            self.lbl_output_path.configure(text=f"出力先(CSV): {out_dir}")
        if hasattr(self, "lbl_plc_shot_path"):
            self.lbl_plc_shot_path.configure(text=f"PLC保存先(画像): {plc_dir}")

    def _ensure_directory_writable(self, path, label):
        if not path:
            return True
        try:
            os.makedirs(path, exist_ok=True)
            test = os.path.join(path, f".write_test_{time.time_ns()}")
            with open(test, "w", encoding="utf-8") as f:
                f.write("ok")
            os.remove(test)
            return True
        except Exception as e:
            self._post_status(f"{label} への書込み確認失敗: {e}")
            return False

    def _save_analysis_preset(self):
        ...
        with open(path, "w", encoding="utf-8") as f:
            json.dump(payload, f, ensure_ascii=False, indent=2)
        self._post_status(f"解析プリセット保存: {path}")

    def _load_analysis_preset(self):
        ...
        for i in range(min(4, len(cams))):
            src = cams[i]
            v = self.cam_vars[i]
            for k in ["flip_horizontal", "trim_ratio_x", "trim_ratio_y", "diff_thresh", "band_top", "band_right", "band_bottom", "corner_exclude_x_px", "corner_exclude_y_px"]:
                if k in src:
                    v[k].set(src[k])
        self._post_status(f"解析プリセット読込: {path}")
    ...
    def _open_camera_settings(self):
        ...
        for i in range(4):
            ...
            v = self.cam_vars[i]
            ttk.Label(tab, text="Trig=このカメラの開始信号 / RU,RD=判定結果 / DONE=完了パルス", bootstyle=INFO).pack(fill="x", pady=(0, 2))
            r2 = ttk.Frame(tab); r2.pack(fill="x", pady=2)
            ttk.Label(r2, text="Trig").pack(side="left")
            ttk.Entry(r2, textvariable=v["dev_trig"], width=10).pack(side="left", padx=4)
            ...
            ttk.Label(tab, text="ERR_TO=撮像失敗 / ERR_AN=解析失敗 / CSV=このカメラ結果ファイル名", bootstyle=INFO).pack(fill="x", pady=(2, 0))
    ...
    def _build_cam_tab(self, parent, v, cam_idx):
        ...
        ctrl = ttk.Labelframe(paned, text="画像処理パラメータ", padding=8)

        r1 = ttk.Frame(ctrl); r1.pack(fill="x", pady=2)
        ttk.Checkbutton(r1, text="左右反転", variable=v["flip_horizontal"]).pack(side="left", padx=10)
        ttk.Label(ctrl, text="trim系=検査ROI比率 / band系=辺抽出幅 / ex系=角除外px", bootstyle=INFO).pack(fill="x", pady=(0, 2))

        r2 = ttk.Frame(ctrl); r2.pack(fill="x", pady=2)
        r2.columnconfigure(1, weight=1)
        r2.columnconfigure(3, weight=1)
        fields = [
            ("trim_ratio_x", "trim_x", 8),
            ("trim_ratio_y", "trim_y", 8),
            ("diff_thresh", "th", 10),
            ("band_top", "band_top", 8),
            ("band_right", "band_right", 8),
            ("band_bottom", "band_bottom", 8),
            ("corner_exclude_x_px", "ex_x", 8),
            ("corner_exclude_y_px", "ex_y", 8),
        ]
        for i in range(0, len(fields), 2):
            row = i // 2
            key1, label1, w1 = fields[i]
            ttk.Label(r2, text=label1).grid(row=row, column=0, sticky="e", padx=(0, 4), pady=2)
            ttk.Entry(r2, textvariable=v[key1], width=w1).grid(row=row, column=1, sticky="we", padx=(0, 8), pady=2)
            key2, label2, w2 = fields[i + 1]
            ttk.Label(r2, text=label2).grid(row=row, column=2, sticky="e", padx=(0, 4), pady=2)
            ttk.Entry(r2, textvariable=v[key2], width=w2).grid(row=row, column=3, sticky="we", pady=2)

        r5 = ttk.Frame(ctrl); r5.pack(fill="x", pady=(8, 4))
        ttk.Button(r5, text="画像を選択して解析", bootstyle=INFO, command=lambda i=cam_idx: self.on_cam_analyze_file(i)).pack(side="left")
        ttk.Button(r5, text="撮像", bootstyle=PRIMARY, command=lambda i=cam_idx: self.on_cam_snap(i)).pack(side="left", padx=(4, 0))
        ...
        preview = ttk.Labelframe(paned, text="プレビュー", padding=6)
        paned.add(ctrl, weight=1)
        paned.add(preview, weight=3)
        try:
            paned.pane(ctrl, minsize=320)
            paned.pane(preview, minsize=520)
        except tk.TclError:
            # Tkバリアント差異で pane の minsize 指定が使えない場合は可変幅のみ有効化
            pass

        canvas = ZoomPanCanvas(preview, bg="#111111", highlightthickness=1, highlightbackground="#444444")
        ...
        self.cam_panels.append({"canvas": canvas, "result_var": result_var})

    def _cam_analysis_kwargs(self, cam_idx):
    ...
    def _analyze_path_for_cam(self, cam_idx, path):
        return analyze_image(path, **self._cam_analysis_kwargs(cam_idx))
    ...
    def _analyze_bgr_for_cam(self, cam_idx, bgr):
        ...
        buf.tofile(tmp_path)
        try:
            return self._analyze_path_for_cam(cam_idx, tmp_path)
        finally:
            try:
                os.remove(tmp_path)
            except Exception:
    ...
    def _result_text_from_analysis(self, res):
        return (
    ...
    def on_cam_analyze_file(self, cam_idx):
        path = filedialog.askopenfilename(
            title=f"Cam{cam_idx+1}: 解析する画像を選択",
            filetypes=[("画像ファイル", "*.bmp;*.jpg;*.jpeg;*.png"), ("すべてのファイル", "*.*")],
        )
        if not path:
            return
        self._last_file_paths[cam_idx] = path
        try:
            self._analyze_file_for_cam(cam_idx, path)
            self._post_status(f"Cam{cam_idx+1} ファイル解析完了: {os.path.basename(path)}")
        except Exception as e:
            raw = imread_unicode(path)
            if raw is not None:
                self._post_cam_preview(cam_idx, raw, f"解析エラー: {e}")
            self._post_status(f"Cam{cam_idx+1} ファイル解析エラー: {e}")

    def _analyze_file_for_cam(self, cam_idx, path, from_auto=False):
        self._sync_cfg()
        res = self._analyze_path_for_cam(cam_idx, path)
        txt = self._result_text_from_analysis(res)
        if from_auto:
            txt = f"[再解析] {txt}"
        self._post_cam_preview(cam_idx, res["img_bgr"], txt)

    def _schedule_file_reanalysis(self, cam_idx):
        path = self._last_file_paths[cam_idx]
        if not path or not os.path.isfile(path):
            return
        if self.cam_grab_threads[cam_idx] and self.cam_grab_threads[cam_idx].is_alive():
            return
        job = self._reanalysis_jobs[cam_idx]
        if job:
            try:
        ...
        def _run():
            self._reanalysis_jobs[cam_idx] = None
            try:
                self._analyze_file_for_cam(cam_idx, path, from_auto=True)
                self._post_status(f"Cam{cam_idx+1} パラメータ変更で再解析: {os.path.basename(path)}")
            except Exception as e:
                raw = imread_unicode(path)
                if raw is not None:
                    self._post_cam_preview(cam_idx, raw, f"再解析エラー: {e}")
                self._post_status(f"Cam{cam_idx+1} 再解析エラー: {e}")

        self._reanalysis_jobs[cam_idx] = self.after(200, _run)

    def _run_cam_snap_and_preview(self, cam_idx):
        self._last_file_paths[cam_idx] = None
    ...
    def _bind_traces(self):
        ...
        vars_all = [
            self.var_plc_ip, self.var_plc_port, self.var_dev_busy,
            self.var_dev_alive, self.var_alive_ms, self.var_alive_step,
        ...
        self.var_output_dir.trace_add("write", lambda *_: self._refresh_path_labels())
        self.var_plc_shot_dir.trace_add("write", lambda *_: self._refresh_path_labels())

        param_keys = [
            "flip_horizontal", "trim_ratio_x", "trim_ratio_y", "diff_thresh",
            "band_top", "band_right", "band_bottom", "corner_exclude_x_px", "corner_exclude_y_px",
        ]
        for cam_idx, v in enumerate(self.cam_vars):
            for key in param_keys:
                v[key].trace_add("write", lambda *_args, i=cam_idx: self._schedule_file_reanalysis(i))
    ...
    def _post_status(self, text):
        ...
        self._status_lines.appendleft(line)
    ...
    def _hb_start(self):
        ...
        try:
            with self._plc_lock:
                self.hb_value = self.plc.read_word(self.cfg.dev_alive)
        except Exception:
            self.hb_value = 0
        self.hb_thread = threading.Thread(target=self._hb_loop, daemon=True)
        self.hb_thread.start()
        self._post_status("HB開始")

    def _hb_stop_thread(self):
        self.hb_stop = True
        if self.hb_thread and self.hb_thread.is_alive():
    ...
    def _hb_loop(self):
        while not self.hb_stop:
            if not self.plc_connected:
                ...
                continue
            try:
                step = max(1, int(self.cfg.alive_step))
                self.hb_value = (self.hb_value + step) & 0xFFFF
                with self._plc_lock:
                    self.plc.write_word(self.cfg.dev_alive, self.hb_value)
            except Exception as e:
                self._post_status(f"HB書込エラー: {e}")
            time.sleep(max(10, int(self.cfg.alive_ms)) / 1000.0)
    ...
    def on_watch_start(self):
        if not self.plc_connected:
            ...
            return
        self._sync_cfg()
        out_ok = self._ensure_directory_writable(self.cfg.output_dir, "出力先")
        plc_ok = self._ensure_directory_writable(self.cfg.plc_shot_dir, "PLC保存先")
        if not (out_ok and plc_ok):
            self._post_status("保存先設定を確認してください")
            return
        self.plc_stop = False
        self.prev_trigs = [False, False, False, False]
        ...
        self.trigger_worker_thread = threading.Thread(target=self._trigger_worker_loop, daemon=True)
        self.plc_thread.start()
        self.trigger_worker_thread.start()
    ...
    def on_watch_stop(self):
        ...
        self.plc_thread = None
        if self.trigger_worker_thread and self.trigger_worker_thread.is_alive():
            self.trigger_worker_thread.join(timeout=1.0)
        self.trigger_worker_thread = None
    ...
    def _plc_loop(self):
        while not self.plc_stop:
            for i, c in enumerate(self.cfg.cams):
                try:
                    with self._plc_lock:
                        trig = self.plc.read_bit(c.dev_trig)
                except Exception as e:
                    self._post_status(f"Cam{i+1} Trig読取エラー: {e}")
                    trig = False

                if trig and not self.prev_trigs[i]:
                    try:
                        self.trigger_queue.put_nowait(i)
                    except queue.Full:
                        self._post_status("トリガキュー満杯: 新規トリガを破棄")

                self.prev_trigs[i] = trig
    ...
    def _trigger_worker_loop(self):
        while not self.plc_stop:
            try:
                cam_idx = self.trigger_queue.get(timeout=0.2)
            except queue.Empty:
                continue
            self._process_trigger_for_camera(cam_idx)

    def _process_trigger_for_camera(self, cam_idx):
        c = self.cfg.cams[cam_idx]
        self._post_status(f"Cam{cam_idx+1} トリガ検出")
        try:
            with self._plc_lock:
            ...
            try:
                ...
                shot_path = os.path.join(shot_dir, f"PLCshot_cam{cam_idx+1}_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}.png")
                ok, buf = cv2.imencode('.png', bgr)
                if not ok:
                    raise RuntimeError("画像エンコード失敗")
                ...
                err_msg = f"CAPTURE:{e}"
                shot_path = ""
                self._post_status(f"Cam{cam_idx+1} 撮像エラー: {e}")

            if shot_path:
                try:
                    res = self._analyze_path_for_cam(cam_idx, shot_path)
                    ru = (res["ru_result"] == "NOTCH")
                    rd = (res["rd_result"] == "NOTCH")
                    ru_area = int(res["ru_area"])
                    rd_area = int(res["rd_area"])
                    self._post_cam_preview(cam_idx, res["img_bgr"], self._result_text_from_analysis(res))
                except Exception as e:
                    an_error = True
                    err_msg = (err_msg + " | " if err_msg else "") + f"ANALYZE:{e}"
                    self._post_status(f"Cam{cam_idx+1} 解析エラー: {e}")
                    try:
                        raw = imread_unicode(shot_path)
                        if raw is not None:
                            self._post_cam_preview(cam_idx, raw, f"解析エラー: {e}")
                    except Exception:
                        pass
            ...
            try:
                ...
                self._post_status(f"Cam{cam_idx+1} PLC書込エラー: {e}")

            self._append_cam_csv(cam_idx, c, [
                datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                cam_idx + 1,
                os.path.basename(shot_path) if shot_path else "",
                int(ru), int(rd), ru_area, rd_area, err_msg,
            ])
            self._post_status(f"Cam{cam_idx+1} 返答完了 TO_ERR={int(to_error)} AN_ERR={int(an_error)}")
        finally:
            try:
                with self._plc_lock:
                    self.plc.write_bit(self.cfg.dev_busy, False)
            except Exception as e:
                self._post_status(f"BUSY解除エラー: {e}")

    def _start_auto_manager(self):
        th = threading.Thread(target=self._auto_manager_loop, daemon=True)
        th.start()
    ...
    def _auto_manager_loop(self):
        ...
        while True:
            try:
                if bool(self.cfg.auto_reconnect) and not self.manual_disconnected:
                    if not self.plc_connected:
                        self.on_plc_connect()
                    if self.plc_connected and not (self.plc_thread and self.plc_thread.is_alive()):
                        self.on_watch_start()
                backoff = 1.0
                time.sleep(0.8)
            except Exception as e:
                self._post_status(f"自動再接続処理エラー: {e}")
                time.sleep(backoff)
